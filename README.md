...
> **Check `git diff package.json` after a build that FAILED.** electron-builder rewrites `package.json` into the trimmed form it packages, and restores it when it finishes. A build that dies partway — the `winCodeSign` failure below does exactly this — leaves the trimmed copy in place, silently dropping `scripts`, `devDependencies` and the whole `build` block. The packaged app is unaffected (it needs only `main` and `version`), so nothing looks wrong until `npm test` reports *"Missing script: test"*. A clean build does not do this. Restore with `git checkout package.json`.
